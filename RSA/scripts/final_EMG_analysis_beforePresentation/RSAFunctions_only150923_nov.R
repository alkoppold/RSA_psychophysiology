...
RSMVector<-function(OriginalData,nVariables,CharContainingNumberFromVector,VariableType,sortType){
  ...
  plotRSM<-ggplot(data=corr_prepPlot, aes(x = var1, y = var2, fill = mean_all)) + #value2
    geom_tile() +
    labs(x = paste("Trials sorted by", sortType), y = paste("Trials sorted by", sortType), fill = "", title = paste("Similarity Matrix based on", VariableType)) +
    coord_fixed() +    
    theme_minimal()+
  ...
  RSMName<-paste(VariableType,sortType,"RSM",sep= "_")
  assign( RSMName,plotRSM,envir = globalenv())
  return(plotRSM)
...
# MaxValue<- max(PicSTRVal$T_scores, na.rm=T)
# 
# OriginalData<-PicEda
# nVariables<-18 # this variable defines the number of cells e.g., pictures presented
# CharContainingNumberFromVector<-4 #Character containing number from variables coding vector.
...
RSMSingleValue<-function(OriginalData,nVariables,CharContainingNumberFromVector,VariableType,sortType,MaxValue){
  ...
  corr_prep_all<-corr_prep_all%>% 
    mutate(mean_all= rowMeans(dplyr::select(., 3:ncol(corr_prep_all)),dims = 1, na.rm=TRUE))
...
RSMSingleValueWeighted<-function(OriginalData,nVariables,CharContainingNumberFromVector,VariableType,sortType,MaxValue){
  ...
  for(i in 1: length(Vp_count$VP)) {
    ...
    for(ii in 1:(length(gaSingle)-1)) {
      for (iii in 1:(length(gaSingle)-1)) {
        # corrMatrixVpSingle[ii,iii]= abs(gaSingle[1,ii+1]-gaSingle[1,iii+1])
        corrMatrixVpSingle[ii,iii]= MaxValue-
          abs(gaSingle[1,ii+1]-gaSingle[1,iii+1])/(1+gaSingle[1,ii+1]+gaSingle[1,iii+1])
...
RSMSingleValueFactor<-function(OriginalData,nVariables,CharContainingNumberFromVector,VariableType,sortType,MaxValue){
  ...
  max<-ceiling_dec( max(corr_prepPlot[,3][corr_prepPlot[,3] != max(corr_prepPlot[,3])]),5) 
  if(max==minimum){max<-max(corr_prepPlot[,3])}
...
cols <- rev(rainbow(20)[-20])
  ...
  plotRSM<-ggplot(data=corr_prepPlot, aes(x = var1, y = var2, fill = mean_all)) + #value2
    ...
    scale_y_continuous(trans = "reverse")+
    scale_fill_gradientn(colours=rev(mypalette),na.value = colors() [c(73)], #heat.colors(12)
                         breaks=c(minimum, max),labels=c(minimum, max),
                         limits=c(minimum,max))+ theme(legend.position="bottom")+ 
    theme(text=element_text(family="Times", size=12))
...
#Participant= if individual participants are to be selected, 
#default= mean_all
# sortType #variable to sort on

PlotMatrix<-function(dataOriginal, minimum,maximum,Category,sortType, Participant,mypalette){
...
mypalette<- colors()[c(73, 30, 28, 124, 635, 86,384,144, 148)]
  ...
  if (length(Participant)==0){
    Participant= "mean_all"
  }   
    
    corr_prepPlot<- dataOriginal%>%
  dplyr::select(var1, var2, Participant)
    names(corr_prepPlot)<- c("var1", "var2","mean_all")
if (length(minimum)==0){
minimum<- floor_dec(min(corr_prepPlot[,3]),5)
}
if (length(maximum)==0){
maximum<-ceiling_dec( max(corr_prepPlot[,3][corr_prepPlot[,3] != max(corr_prepPlot[,3])]),5) 
if(maximum==minimum){maximum<-max(corr_prepPlot[,3])}
}
cols <- rev(rainbow(20)[-20])
...
PlotMatrixCombined<-function(dataOriginal, minimum,maximum,Category,sortType, Participant,mypalette){
...
corr_prepPlot<- dataOriginal%>%
  dplyr::select(var1, var2, Participant)
names(corr_prepPlot)<- c("var1", "var2","Similarity")
corr_prepPlotU<-corr_prepPlot%>% filter(var1>var2)
minimum<- floor_dec(min(corr_prepPlotU[,3]),5)
maximum<-ceiling_dec( max(corr_prepPlotU[,3][corr_prepPlotU[,3] != max(corr_prepPlotU[,3])]),5) 
corr_prepPlotD<-corr_prepPlot%>% filter(var1<var2)
minimumd<- floor_dec(min(corr_prepPlotD[,3]),5)
maximumd<-ceiling_dec( max(corr_prepPlotD[,3][corr_prepPlotD[,3] != max(corr_prepPlotD[,3])]),5) 
mypalette<- colors()[c(73, 30, 28, 124, 635, 86,384,144, 148)]

cols <- rev(rainbow(20)[-20])

plotRSM<-ggplot(data=corr_prepPlot, aes(x = var1, y = var2, fill = Similarity)) + #value2
  
  geom_tile(aes(fill = Similarity), data = ~ subset(.x, var1 > var2)) +
  scale_y_continuous(trans = "reverse")+
  scale_fill_gradientn(colours=rev(mypalette),na.value = colors() [c(73)], #heat.colors(12)
                       breaks=c(minimum, maximum),labels=c(minimum, maximum),
  ...
  geom_tile(aes(fill = Similarity), data = ~ subset(.x, var1 < var2)) +
  scale_fill_gradientn(colours=rev(mypalette),na.value = colors() [c(73)], #heat.colors(12)
                       breaks=c(minimumd, maximumd),labels=c(minimumd, maximumd),
                       limits=c(minimumd,maximumd))+ 
...
# ModelData<-SCR_Vector_corr_prep_all
...
the BF10 index was: {bfevidence} "
  ...
  mylist<-list (plot.perm, template2)
    # return(mylist)
  names(mylist)<-c("PermutationPlot","Summary")
  listFinal<-list(VariableOriginal, VariableModel,test.cor,permPValue, Perm_test_corr_sort,Perm.test.corr,bf,mylist)
  names(listFinal)<- c("VariableOriginal", "VariableModel","Similarity","p-value", "SimilarityMatrix","PermutationTest","bf","Output")
   PermName<-paste(VariableOriginal,VariableModel,"PermOutput",sep= "_")
  assign(PermName,listFinal,envir = globalenv())
...
# 
# tableData<-STR_corr_prep_all
# tableModel<-Behav_Single_STR_corr_prep_all
...
#     if(length(tableDaugther2[,which( colnames(tableDaugther2)==names(tableMother2[i]))])>0){
...
#              col="blue", size=5, parse=TRUE)+
#     ylab(paste("Similarity between", Model, "&", Data) ) # for the y axis label
#   plotDistribution
#   IndName<-paste(Model,Data,"IndividualPlot",sep= "_")
#   assign(IndName,plotDistribution,envir = globalenv())
#   
# 
#   }
...
RSMIndividual<-function(tableModel,tableData,Model,Data){
  ...
  for(i in 3:(ncol(tableMother)-1)) { #starts in column three
    ...
    single<-tableMother2[,c(1,2,i)]
    if(length(tableDaugther2[,which( colnames(tableDaugther2)==names(tableMother2[i]))])>0){
      single2<-tableDaugther2[,c(1,2,which( colnames(tableDaugther2)==names(tableMother2[i])))]
      single3<-inner_join(single,single2,by = c("var1","var2"))
      corrspSingle<- rcorr(as.matrix(single3[,3:4]), type="spearman")
      tableCorrelation[i-2,2]<-corrspSingle$r[2]
      tableCorrelation[i-2,1]<-names(tableMother2[i])
    }
  }
...
RSMIndividualRegression<-function(OriginalData,tableModel1,tableModel2,VariableOriginal,VariableModel1,VariableModel2){ 
  #Select the biggest table
  # if (ncol(tableModel1)>ncol(OriginalData)){
    tableMother<-OriginalData
    tableDaugther<-tableModel1
  # }else {tableMother<-tableModel1
  # tableDaugther<-OriginalData}
  ...
  names(tableCorrelation)<-c("VP",paste("Similarity",VariableOriginal,VariableModel1,sep="_"),
                             paste("Similarity",VariableOriginal,VariableModel2,sep="_"))
  tableDaugther2<-tableDaugther%>%
    filter(tableDaugther$var1>tableDaugther$var2)
  tableMother2<-tableMother%>%
    filter(tableMother$var1>tableMother$var2)
  ...
  for(i in 3:(ncol(tableMother)-1)) { #starts in column three
    ...
    # if(length(tableDaugther2[,which( colnames(tableDaugther2)==names(tableMother2[i]))])>0){
    if(length(tableDaugther2[,which( colnames(tableDaugther2)==names(tableMother2[i]))])>0 &&
       length(tableCousin2[,which( colnames(tableCousin2)==names(tableMother2[i]))])>0){
      single[,2]<-tableDaugther2[,which( colnames(tableDaugther2)==names(tableMother2[i]))]
    ...
    if(length(modelM1[["residuals"]])== nrow(single)){
      corrspSingleM1[,2]<-modelM1[["residuals"]]}
    if(length(modelM2[["residuals"]])== nrow(single)){
      corrspSingleM1[,3]<-modelM2[["residuals"]]}
    corrspSingleM1<-corrspSingleM1%>%drop_na()
    if (nrow(corrspSingleM1)>4){
  ...
  plotDistributionM1 <- ggplot(tableCorrelationPlot, aes_string(y = names(tableCorrelationPlot[1]),x = "x")) +
    ...
    ylab(paste("Similarity between", VariableOriginal, "&", VariableModel1) ) # for the y axis label
  ...
  plotDistributionM2 <- ggplot(tableCorrelationPlot, aes_string(y = names(tableCorrelationPlot[2]),x = "x")) +
    ...
    ylab(paste("Similarity between", VariableOriginal, "&", VariableModel2) ) # for the y axis label
  ...
  listFinal<-list(plotDistributionM1, plotDistributionM2)
  IndName<-paste(VariableOriginal,VariableModel1,VariableModel2,"IndividualPlotRegression",sep= "_")
  assign(IndName,listFinal,envir = globalenv())
  
}
#----------------------------------------------------------------------------
#----------------------------------------------------------------------------
#-----------------------------Permutattion test with regression-----------------------------------------------
#----------------------------------------------------------------------------
...
the BF10 index was: {bfevidence} "
    ...
    listM1<-list(VariableOriginal, VariableModel1,test.cor,permPValue, Perm_test_corr_sort,Perm.test.corr,bf, plotRSM1)
    # listFinal<-list(VariableOriginal, VariableModel,test.cor,permPValue, Perm_test_corr_sort,Perm.test.corr,bf,mylist)
    names(listM1)<- c("VariableOriginal", "VariableModel","Similarity","p-value", "SimilarityMatrix","PermutationTest","bf","PermutationPlot")
...
the BF10 index was: {bfevidence} "
  ...
  #}
  listM2<-list(VariableOriginal, VariableModel2,test.cor,permPValue, Perm_test_corr_sort,Perm.test.corr,bf, plotRSM2)
  names(listM2)<- c("VariableOriginal", "VariableModel","Similarity","p-value", "SimilarityMatrix","PermutationTest","bf","PermutationPlot")
  ...
  names(mylist)<-c("PermutationPlotM1", "RSMPlotRegressionM1", "SummaryM1","PermutationPlotM2", "RSMPlotRegressionM2", "SummaryM2" )
 
   listFinal<-list(listM1, listM2,mylist)
  names(listFinal)<- c("Model1", "Model2", "Ouput")
  PermName<-paste(VariableOriginal,VariableModel1,VariableModel2,"PermOutputRegr",sep= "_")
  assign(PermName,listFinal,envir = globalenv())
  # return(mylist)
