...
loadData<-1
if (loadData==1){
  load("RSA_STR_Pic_VB22_Def.RData")
  loadData<-1
}
#Set the wd
...
PicSTRVal<-STR_RSM  %>%
  filter(Task=="1")
PicSTRVal<-PicSTRVal%>%droplevels()
PicSTRVal$Category<-ifelse(PicSTRVal$trial_type==8,"Pl",
                           ifelse(PicSTRVal$trial_type==10,"Un","Ne"))
PicSTRValMod<-PicSTRVal%>%
...
RSMSingleValue(PicSTRVal,nVariables,CharContainingNumberFromVector,VariableType,sortType,MaxValue)
}
```
...
if (loadData==0){
  PicBehaValsV<-STR_RSM  %>%
  filter(Task==1)%>%
  dplyr::select(Vp, sortVal,Val)
PicBehaValsV<-PicBehaValsV%>%
  rename(Behavioral= Val)%>%
  rename(sort= sortVal)

PicBehaValsV<-PicBehaValsV%>%droplevels()
# OriginalData<-PicBehaValsV
MaxValue<- max(PicBehaValsV$Behavioral, na.rm=T)
...
RSMSingleValue(PicBehaValsV,nVariables,CharContainingNumberFromVector,VariableType,sortType, MaxValue)

VariableType<-"Behav_Single_STR_AK"
sortType<- "Valence"

RSMSingleValueAK(PicBehaValsV,nVariables,CharContainingNumberFromVector,VariableType,sortType, MaxValue)

VariableType<-"Behav_Single_STR_invAK"
sortType<- "Valence"

RSMSingleValueinvAK(PicBehaValsV,nVariables,CharContainingNumberFromVector,VariableType,sortType, MaxValue)
...
Participant<- "VP1" # factor() #if individual participants are to be selected,
...
maximum<-numeric()# scale
Category<- "trials" # VP or trials
Participant<- "VP28" # factor() #if individual participants are to be selected,
sortType<- "Valence" #variable to sort on
PlotMatrix(dataOriginal, minimum,maximum,Category,sortType,Participant, mypalette)
```
...
maximum<-numeric()# scale
Category<- "trials" # VP or trials
Participant<- "VP8" # factor() #if individual participants are to be selected,
sortType<- "Valence" #variable to sort on
PlotMatrix(dataOriginal, minimum,maximum,Category,sortType,Participant, mypalette)
...
if (loadData==0){
  #----------- MODEL TIME-----------------------
  #Create Matrix based on single BEHAVIORAL
  PicSTRValsVTime<-STR_RSM  %>%
  filter(Task==1)%>%
  dplyr::select(Vp, sortVal,trial_order)%>%droplevels()
PicSTRValsVTime<-PicSTRValsVTime%>%
  rename(Behavioral= trial_order)%>%
  rename(sort= sortVal)
PicSTRValsVTime$Behavioral<-as.numeric(PicSTRValsVTime$Behavioral)
  # OriginalData<-PicEdaArosV
  MaxValue<- max(PicSTRValsVTime$Behavioral, na.rm=T)
  ...
  RSMSingleValue(PicSTRValsVTime,nVariables,CharContainingNumberFromVector,VariableType,sortType, MaxValue)
...
if (loadData==0){
  #----------------------------- MODEL Category --------------------------------------
 PicSTRValsVCat<-STR_RSM  %>%
  filter(Task==1)

  PicSTRValsVCat$Category<-ifelse(PicSTRValsVCat$trial_type==8,"Pl",
                                    ifelse(PicSTRValsVCat$trial_type==10,"Un","Ne"))
 
   PicSTRValsVCat<-PicSTRValsVCat%>%
  ...
  RSMSingleValueFactor(PicSTRValsVCat,nVariables,CharContainingNumberFromVector,VariableType,sortType, MaxValue)
...
if (loadData==0){
  ...
  PicSTRValsVCat$Category2<-ifelse(PicSTRValsVCat$Behavioral=="Ne","No","Ye")
  PicEdaArosVCat2<-PicSTRValsVCat  %>%
...
ggarrange(STR_Valence_AK_Category2_Factor_IndividualPlotRegression[[1]],
          ...
          labels = c("", "" ),   vjust=-7,
          ncol = 2, nrow =1, widths = c(5,5))
save.image("RSA_STR_Pic_VB22_Def.RData")
