...
loadData<-1
if (loadData==1){
  load("RSA_SCR_Sound_VB22_Def.RData")
loadData<-1
}
#Set the wd
...
#------------------------
#Select Task Pasive Sound Listening
SoundEdaAro<-SCR_RSM  %>%
  filter(Task_def==2)
SoundEdaAro$Category<-ifelse(SoundEdaAro$trial_type==2,"Pl",
                           ifelse(SoundEdaAro$trial_type==4,"Un","Ne"))
SoundEdaAroMod<-SoundEdaAro%>%
...
if (loadData==0){
SoundBehaArosV<-SCR_RSM  %>%
  filter(Task_def==2)%>%
  dplyr::select(Vp_ratings, sortAro,Aro)
SoundBehaArosV<-SoundBehaArosV%>%
  rename(Behavioral= Aro)%>%
  rename(sort= sortAro)
# OriginalData<-SoundBehaArosV
MaxValue<- max(SoundBehaArosV$Behavioral, na.rm=T)
...
RSMSingleValue(SoundBehaArosV,nVariables,CharContainingNumberFromVector,VariableType,sortType, MaxValue)

VariableType<-"Behav_Single_AK"
sortType<- "Arousal"

RSMSingleValueAK(SoundBehaArosV,nVariables,CharContainingNumberFromVector,VariableType,sortType, MaxValue)

VariableType<-"Behav_Single_invAK"
sortType<- "Arousal"

RSMSingleValueinvAK(SoundBehaArosV,nVariables,CharContainingNumberFromVector,VariableType,sortType, MaxValue)
...
if (loadData==0){
  #----------- MODEL TIME-----------------------
#Create Matrix based on single BEHAVIORAL
SoundBehavArosVTime<-SCR_RSM  %>%
  filter(Task_def==2)%>%
  dplyr::select(Vp_ratings, sortAro,trial_order)
SoundBehavArosVTime<-SoundBehavArosVTime%>%
  rename(Behavioral= trial_order)%>%
  rename(sort= sortAro)
# OriginalData<-SoundEdaArosV
MaxValue<- max(SoundBehavArosVTime$Behavioral, na.rm=T)
...
RSMSingleValue(SoundBehavArosVTime,nVariables,CharContainingNumberFromVector,VariableType,sortType, MaxValue)
...
if (loadData==0){
 #----------------------------- MODEL Category --------------------------------------
SoundBehavArosVCat<-SCR_RSM  %>%
  filter(Task_def==2)
  SoundBehavArosVCat$Category<-ifelse(SoundBehavArosVCat$trial_type==2,"Pl",
                           ifelse(SoundBehavArosVCat$trial_type==4,"Un","Ne"))
    SoundBehavArosVCat<-SoundBehavArosVCat%>%
    dplyr::select(Vp_ratings, sortAro,length(SoundBehavArosVCat))
...
RSMSingleValueFactor(SoundBehavArosVCat,nVariables,CharContainingNumberFromVector,VariableType,sortType, MaxValue)
...
SoundBehavArosVCat$Category2<-ifelse(SoundBehavArosVCat$Behavioral=="Ne","No","Ye")
SoundEdaArosVCat2<-SoundBehavArosVCat  %>%
